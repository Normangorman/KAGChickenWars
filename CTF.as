...
void Config(CTFCore@ this)
{
	...
	*/
	...
	getRules().set_u8("init_chickens", cfg.read_u8("init_chickens", 4));
	getRules().set_u16("chickens_to_win", cfg.read_u16("chickens_to_win", 200));
	log("Config", "init_chickens " + getRules().get_u8("init_chickens"));
	log("Config", "chickens_to_win " + getRules().get_u16("chickens_to_win"));
...
shared class CTFCore : RulesCore
{
	...
	void Update()
	{
		...
		if (getGameTime() % CHECK_TEAM_WON_FREQ == 0) {
			CheckTeamWon();
		}
	}
	...
	void SetupBases()
	{
		...
		if (map !is null)
		{
			...
			SpawnInitChickens(respawnPos - Vec2f(0,4), 0);
			...
			SpawnInitChickens(respawnPos - Vec2f(0,4), 1);
	...
	void CheckTeamWon()
	{
		...
		// Count chickens
		int team0Chickens = GetChickenCountForTeam(0);
		int team1Chickens = GetChickenCountForTeam(1);

		int winteamIndex = -1;
		CTFTeamInfo@ winteam = null;
		s8 team_wins_on_end = -1;
		u16 chickens_to_win = getRules().get_u16("chickens_to_win");
		...
		for (uint team_num = 0; team_num < teams.length; ++team_num)
		{
			...
			bool win = true;
			if (team_num == 0 && team0Chickens >= chickens_to_win) {
				winteamIndex = team_num;
				@winteam = team;
			}
			else if (team_num == 1 && team1Chickens >= chickens_to_win) {
				winteamIndex = team_num;
				@winteam = team;
			}
		}
...
{
	Reset(this);
	this.set_s32("restart_rules_after_game_time", 17 * 30); // 17 to align with yakety sax music
}
...
// Called once for each base at the start of the game
shared void SpawnInitChickens(Vec2f pos, int teamNum) {
	log("SpawnInitChickens", "Spawning for " + teamNum);
	u8 init_chickens = getRules().get_u8("init_chickens");
	for (int i=0; i < init_chickens; ++i) {
		Vec2f offset(XORRandom(16) - 8, 0); // prevent tham all spawning on exactly the same spot
		server_CreateBlob("chicken", teamNum, pos + offset);
	}
}
